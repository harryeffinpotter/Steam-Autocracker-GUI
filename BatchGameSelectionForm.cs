...
namespace SteamAutocrackGUI
{
	...
	public partial class BatchGameSelectionForm : Form
	{
		...
		private void BuildUi()
		{
			...
			try { infoIcon = APPID.Properties.Resources.info_icon; } catch (Exception ex) { Console.WriteLine(ex.Message); }
			try { zipperIcon = APPID.Properties.Resources.zipper_icon; } catch (Exception ex) { Console.WriteLine(ex.Message); }
			try { rinIcon = APPID.Properties.Resources.rin_icon?.ToBitmap(); } catch (Exception ex) { Console.WriteLine(ex.Message); }
			try { discordIcon = APPID.Properties.Resources.discord_icon; } catch (Exception ex) { Console.WriteLine(ex.Message); }
			try { notepadIcon = APPID.Properties.Resources.notepad_icon; } catch (Exception ex) { Console.WriteLine(ex.Message); }
					...
					if (!string.IsNullOrEmpty(gamePath) && finalUrls.ContainsKey(gamePath) && uploadedGames.ContainsKey(gamePath))
					{
						...
						else if (colName == "RinCopy")
						{
							...
							else
							{
								...
								// Build date comes from steamcmd by buildid (not file/install time).
								string versionDate = await APPID.SteamUpdateInfo.GetBuildVersionLabelAsync(game.AppId, game.BuildId);
			...
			// (Compression settings button removed from the batch window - settings
			// are reached from the share sheet's title bar instead.)
		...
		public void UpdateStatus(string gamePath, string status, Color? color = null)
		{
			...
			{
				try { this.BeginInvoke(new Action(() => UpdateStatus(gamePath, status, color))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void UpdateStatusByIndex(int rowIndex, string status, Color? color = null)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => UpdateStatusByIndex(rowIndex, status, color))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void SetConvertingUrl(string gamePath, string oneFichierUrl)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => SetConvertingUrl(gamePath, oneFichierUrl))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void ClearConvertingUrl(string gamePath)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => ClearConvertingUrl(gamePath))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void SetFinalUrl(string gamePath, string finalUrl)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => SetFinalUrl(gamePath, finalUrl))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void SetUploadedGame(string gamePath, BatchGameItem game)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => SetUploadedGame(gamePath, game))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void SetOneFichierUrl(string gamePath, string url)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => SetOneFichierUrl(gamePath, url))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void UpdateSlotProgress(int slotIndex, int percent, long uploadedBytes, long totalBytes, double bytesPerSecond)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => UpdateSlotProgress(slotIndex, percent, uploadedBytes, totalBytes, bytesPerSecond))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void ReleaseUploadSlot(int slotIndex)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => ReleaseUploadSlot(slotIndex))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void ResetSkipCancelState()
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => ResetSkipCancelState())); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void HideUploadDetails()
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => HideUploadDetails())); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void ShowCopyAllButton(string phpBBLinks)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => ShowCopyAllButton(phpBBLinks))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void UpdateTitleProgress(int percent, string phase = null)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => UpdateTitleProgress(percent, phase))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void UpdateProgressWithEta(int percent, double etaSeconds)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => UpdateProgressWithEta(percent, etaSeconds))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void ResetTitle(string text = "Batch Processor")
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => ResetTitle(text))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void SetProcessingMode(bool processing)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => SetProcessingMode(processing))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
		...
		public void UpdatePyDriveUrl(string gamePath, string pydriveUrl)
		{
			...
			if (this.InvokeRequired)
			{
				try { this.BeginInvoke(new Action(() => UpdatePyDriveUrl(gamePath, pydriveUrl))); } catch (Exception ex) { Console.WriteLine(ex.Message); }
				return;
			}
